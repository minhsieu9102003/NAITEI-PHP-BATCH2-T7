                ...
                <div class="rounded-lg border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                    <div class="h-56 w-full">
                        <a href="{{ route('products.show', ['product' => $viewedProduct->product->id]) }}">
                            <img class="mx-auto h-full dark:hidden" src="{{ $viewedProduct->product->image_url }}" alt="{{ $viewedProduct->product->name }}" />
                        </a>
                    </div>
                    <div class="pt-6">
                        <a href="{{ route('products.show', ['product' => $viewedProduct->product->id]) }}" class="text-lg font-semibold leading-tight text-gray-900 hover:underline dark:text-white">{{ $viewedProduct->product->name }}</a>
                        <p class="text-gray-500 mt-2">{{ $viewedProduct->product->description }}</p>
